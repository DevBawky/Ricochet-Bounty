...
public class ScoreObject : MonoBehaviour
{
    ...
    void OnCollisionEnter2D(Collision2D collision)
    {
        ...
        Vector3 hitPosition = collision.contactCount > 0
            ? (Vector3)collision.GetContact(0).point
            : collision.transform.position;
        ApplyScoreFromBall(collision.gameObject, hitPosition);
    }
    ...
    void OnTriggerEnter2D(Collider2D other)
    {
        ...
        hitPosition.z = other.transform.position.z;
        ApplyScoreFromBall(other.gameObject, hitPosition);
    }

    void ApplyScoreFromBall(GameObject hitObject, Vector3 hitPosition)
    {
        ...
        damageManager.ApplyBallData(ballData, hitPosition);
        ApplyBallObjectHitSystems(ballDataManager, hitPosition);
    }

    void ApplyBallObjectHitSystems(BallDataManager ballDataManager, Vector3 hitPosition)
    {
        // 점수 오브젝트에 닿았을 때 공의 내구도와 효과도 함께 처리합니다.
        // DamageManager는 점수만 담당하고, 공의 런타임 상태는 공에 붙은 컴포넌트들이 담당합니다.
        ballDataManager.SpawnCollisionEffect(hitPosition);

        BallEffectController effectController = ballDataManager.GetComponent<BallEffectController>();
        if (effectController != null)
        {
            effectController.TriggerObjectHitEffects(hitPosition);
        }
